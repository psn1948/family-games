        ...
        {completed.map(session => {
          ...
          return (
            <Link
              ...
              className="card p-3 sm:p-4 flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 hover:bg-gray-50 transition-colors group block"
            >
              ...
              {winner && (
                <div className="flex items-center justify-between sm:justify-end gap-2">
                  <p className="text-xs text-gray-500 font-medium sm:hidden">Højeste point</p>
                  <div className="flex items-center gap-2">
                    <span
                      className="w-4 h-4 sm:w-5 sm:h-5 rounded-full shadow-sm"
                      style={{ backgroundColor: winner.member.color }}
                    />
                    <span className="font-bold text-gray-700 text-sm">{winner.member.name}</span>
                    <span className="font-mono font-bold text-indigo-600">{winner.total}</span>
                  </div>
                </div>
              )}
              <span className="hidden sm:inline text-gray-400 group-hover:text-indigo-500 transition-colors text-lg">→</span>
            </Link>
          )
        })}
      </div>
